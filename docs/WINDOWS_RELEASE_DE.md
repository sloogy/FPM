...
Ein nummerierter Tag wie `v1.0.8-rc.2` führt die echten Windows-/Linux-Builds, den Installer und beide LifePlanner-/LiveManager-Module ohne Signier-Keys aus. Die Ergebnisse werden klar als unsigned GitHub-Prerelease veröffentlicht, jedoch ohne `latest.json`. Der exakte finale Tag `v1.0.8` veröffentlicht nach denselben Gates einen normalen unsigned Release mit `latest.json` und `UNSIGNED_RELEASE.txt`. Die `.lpmodule` benötigen bei lokaler Installation eine ausdrückliche Vertrauensbestätigung.

## Enterprise-Freigabe v1.0.8
