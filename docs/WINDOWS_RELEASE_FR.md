...
Un tag numéroté tel que `v1.2.2-rc.2` exécute les véritables builds Windows/Linux, l’installeur et les deux modules LifePlanner/LiveManager sans clés de signature. Les résultats sont publiés comme GitHub Prerelease clairement non signée, sans `latest.json`. Le tag final exact `v1.2.2` publie après les mêmes contrôles une version normale non signée avec `latest.json` et `UNSIGNED_RELEASE.txt`. L’installation locale des fichiers `.lpmodule` exige une confirmation explicite de confiance.

## Validation Enterprise v1.2.2
