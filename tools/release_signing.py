"""Shared Ed25519 and deterministic tree-hash helpers for release artifacts."""
...
def _decode_key(value: str, *, label: str) -> bytes:
    ...
    except Exception as exc:  # pragma: no cover - exact decoder error is implementation detail
        raise ValueError(f"{label} is not valid base64") from exc
    if len(raw) != 32:
        raise ValueError(f"{label} must contain exactly 32 raw Ed25519 bytes")
    return raw
...
def private_key_from_b64(value: str):
    ...
    return Ed25519PrivateKey.from_private_bytes(_decode_key(value, label="private key"))


def public_key_from_b64(value: str):
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PublicKey

    return Ed25519PublicKey.from_public_bytes(_decode_key(value, label="public key"))
...
def key_id(public_key_b64: str) -> str:
    raw = _decode_key(public_key_b64, label="public key")
    return "ed25519:" + hashlib.sha256(raw).hexdigest()[:16]


def sign_b64(data: bytes, private_key_b64: str) -> bytes:
    signature = private_key_from_b64(private_key_b64).sign(data)
