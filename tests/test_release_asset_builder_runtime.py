...
def test_combined_asset_builder_creates_verified_cross_platform_release(tmp_path):
    ...
    module_windows = tmp_path / "fpm_1.0.2_Windows_x86_64.lpmodule"
    module_linux = tmp_path / "fpm_1.0.2_Linux_x86_64.lpmodule"
    ...
    assert manifest["release_tag"] == "v1.0.2"
    ...
    with zipfile.ZipFile(out / "FountainPenManager-v1.0.2-portable-linux.zip") as archive:
    ...
    with zipfile.ZipFile(out / "FountainPenManager-v1.0.2-portable-windows.zip") as archive:
        names = set(archive.namelist())
        assert {"FountainPenManager.exe", "start-windows.cmd", "data/.keep"} <= names

    with zipfile.ZipFile(out / "FountainPenManager_Setup_1.0.2.zip") as archive:
...
def test_prerelease_asset_builder_creates_unsigned_rc_without_update_manifest(tmp_path):
    ...
    expected = {
        "FountainPenManager-v1.0.2-rc.1-portable-windows.zip",
        "FountainPenManager-v1.0.2-rc.1-portable-linux.zip",
        "FountainPenManager_Setup_1.0.2-rc.1.exe",
        "FountainPenManager_Setup_1.0.2-rc.1.zip",
        "fpm_1.0.2_Windows_x86_64.lpmodule",
        "fpm_1.0.2_Linux_x86_64.lpmodule",
    ...
    for module in (module_windows.name, module_linux.name):
        with zipfile.ZipFile(out / module) as archive:
            assert "component.json.sig" not in archive.namelist()
    with zipfile.ZipFile(out / "FountainPenManager_Setup_1.0.2-rc.1.zip") as archive:
        notice = archive.read("WINDOWS_DOWNLOAD_HINWEIS.txt").decode("utf-8")
        assert "nicht digital signiert" in notice
